...
/*   Updated: 2021/10/27 00:54:12 by arbernar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, excess, 0))
		return (NULL);
	...
	if (!excess)
		excess = ft_strdup("");
	if (read_stack(fd, &excess, &stack, &buffer) == 0 && *stack == '\0')
	{
		free(stack);
		return (NULL);
	}
	extract_excess(&stack, &excess, &line);
	free_ptr(stack);
	return (line);
}
